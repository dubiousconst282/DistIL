...
public abstract class Value
{
    ...
    internal void AddUse(User user, int operIdx)
    {
    ...
    internal void RemoveUse(User user, int operIdx, bool removeEvenIfStillBeingUsed = false)
    {
        Assert(user.Operands[operIdx] != this || removeEvenIfStillBeingUsed);
        if (!removeEvenIfStillBeingUsed && Array.IndexOf(user.Operands, this) >= 0) return;
    ...
    internal void RelocUse(Instruction user, int prevOperandIdx, int newOperandIdx)
    {
        //We don't track operand indices yet
    }
    ...
    public void ReplaceUses(Value newValue)
    {
        ...
        if (_users.GetType() == typeof(UserSet)) {
            foreach (var user in _userSet._slots) {
                if (user != null) {
                    ReplaceUse(newValue, user);
                }
            }
        } else {
            ReplaceUse(newValue, _singleUser);
        }
        //Transfer the user slots to the newValue to avoid copying.
        //ReplaceUse() already handles this.
        if (newValue._users == null) {
            newValue._users = _users;
        }
        newValue._numUses += _numUses;
        _users = null!;
        _numUses = 0;
    }
    private void ReplaceUse(Value newValue, User user)
    {
        var opers = user.Operands;
