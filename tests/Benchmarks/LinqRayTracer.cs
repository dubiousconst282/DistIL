//Taken from https://github.com/lukehoban/LINQ-raytracer
//
//Changes:
// - Use `FirstOrDefault(Color.Background)` instead of `DefaultIfEmpty(Color.Background).First()`
// - Materialize `naturalColors` to a list. The original can't be expanded because it is across a non-inlined method. Saves ~40%
using System;
                ...
                select from x in Enumerable.Range(0, screenWidth)
                       ...
                       let computeTraceRay = (Func<Func<TraceRayArgs, Color>, Func<TraceRayArgs, Color>>)
                        (f => traceRayArgs =>
                         (from isect in
                          ...
                          let naturalColors =
                              ...
                              select Color.Plus(Color.Times(isect.Thing.Surface.Diffuse(pos), lcolor),
                                                Color.Times(isect.Thing.Surface.Specular(pos), scolor))).ToList()
                         ...
                         ).FirstOrDefault(Color.Background))
